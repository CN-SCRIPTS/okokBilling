...
local Webhook = 'https://discord.com/api/webhooks/1272215242998546542/JF9mtuOawt-3D6A9VnNRKPa6vHh8KABCR6jABcTIrH0qyjUfXvgnjR9EHHBNOq5YEF96'
...
AddEventHandler("okokBilling:PayInvoice", function(invoice_id)
	...
	MySQL.query('SELECT * FROM okokBilling WHERE id = @id', {
		...
		if playerMoney < invoices.invoice_value then
			...
			TriggerEvent('notifications:sendNotification', _source, "BILLING", "You don't have enough money!", 10000, 'error')
		else
			...
			TriggerEvent('notifications:sendNotification', _source, "BILLING", "Invoice successfully paid!", 10000, 'success')

			if Webhook ~= 'https://discord.com/api/webhooks/1272215242998546542/JF9mtuOawt-3D6A9VnNRKPa6vHh8KABCR6jABcTIrH0qyjUfXvgnjR9EHHBNOq5YEF96' then
...
AddEventHandler("okokBilling:CancelInvoice", function(invoice_id)
	...
	MySQL.query('SELECT * FROM okokBilling WHERE id = @id', {
		...
		TriggerEvent('notifications:sendNotification', _source, "BILLING", "You have cancelled the invoice!", 10000, 'info')
		if Webhook ~= 'https://discord.com/api/webhooks/1272215242998546542/JF9mtuOawt-3D6A9VnNRKPa6vHh8KABCR6jABcTIrH0qyjUfXvgnjR9EHHBNOq5YEF96' then
...
AddEventHandler("okokBilling:CreateInvoice", function(data)
	...
	if Config.LimitDate then
		MySQL.query('INSERT INTO okokBilling (receiver_identifier, receiver_name, author_identifier, author_name, society, society_name, item, invoice_value, status, notes, sent_date, limit_pay_date) VALUES (@receiver_identifier, @receiver_name, @author_identifier, @author_name, @society, @society_name, @item, @invoice_value, @status, @notes, CURRENT_TIMESTAMP(), DATE_ADD(CURRENT_TIMESTAMP(), INTERVAL @limit_pay_date DAY))', {
			...
			TriggerEvent('notifications:sendNotification', target.PlayerData.source, "BILLING", "You have just received a new invoice!", 10000, 'info')
			if Webhook ~= 'https://discord.com/api/webhooks/1272215242998546542/JF9mtuOawt-3D6A9VnNRKPa6vHh8KABCR6jABcTIrH0qyjUfXvgnjR9EHHBNOq5YEF96' then
