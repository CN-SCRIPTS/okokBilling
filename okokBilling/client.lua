...
function SocietyInvoices(society)
	QBCore.Functions.TriggerCallback("okokBilling:GetSocietyInvoices", function(cb, totalInvoices, totalIncome, totalUnpaid, awaitedIncome)
		if json.encode(cb) ~= '[]' then
			...
			TriggerEvent('notifications:sendNotification', text,"Your society doesn't have invoices.", 10000, 'info')
...
RegisterNUICallback("action", function(data, cb)
	...
	elseif data.action == "createInvoice" then
		...
		if closestPlayer == -1 or playerDistance > 3.0 then
			...
			TriggerEvent('notifications:sendNotification', text,"Error sending the invoice! There's nobody near you.", 10000, 'error')
		else
			...
			TriggerEvent('notifications:sendNotification', text,"Invoice successfully sent!", 10000, 'success')
		end
	...
	elseif data.action == "missingInfo" then
		...
		TriggerEvent('notifications:sendNotification', text,"Fill in all fields before sending an invoice!", 10000, 'error')
	elseif data.action == "negativeAmount" then
		--exports['okokNotify']:Alert("BILLING", "You need to set a positive amount!", 10000, 'error')
		TriggerEvent('notifications:sendNotification', text,"You need to set a positive amount!", 10000, 'error')
		...
		for k, v in pairs(Config.AllowedSocieties) do
			if v == PlayerData.job.name then
				...
				elseif Config.OnlyBossCanAccessSocietyInvoices then
					...
					TriggerEvent('notifications:sendNotification', text,"Only the boss can access the society invoices.", 10000, 'error')
				end
			end
		end
